...
pub fn greet(name: &str) -> String {
    format!("Hello, {}! Welcome to Miden VM Playground!", name)
}
...
pub fn instantiate() -> String {
    ...
    let program = assembler.assemble_program("begin push.8 push.5 add swap drop end").unwrap();
    ...
    let stack_top: Vec<_> = trace.stack_outputs().stack_truncated(1).iter().map(|f| f.to_string()).collect();
...
#[tauri::command]
pub fn exec_program_with_inputs(program: &str, inputs_json: Option<String>) -> Result<String, String> {
    let result = execute_program_impl(program, inputs_json.as_deref());
    Ok(serde_json::to_string(&result).unwrap())
}

#[tauri::command]
pub fn generate_proof_with_inputs(program: &str, inputs_json: Option<String>) -> Result<String, String> {
